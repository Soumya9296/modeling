...
if __name__ == "__main__":
    ...
    core_composition = Composition({'Fe': 86.20, 'Ni': 2.00, 'S': 11.45, 'Si': 0.00, 'O': 0.25, 'C': 0.05, 'H': 0.05}, 'weight')
    ...
    core = burnman.Layer("core", radii=np.linspace(0, 1691.645e3, 100)) #s changed radius from Kh22
    ...
    core.set_temperature_mode("adiabatic", temperature_top = 2550.0) #s 2100 frm S19 in Kh22
...
# Create composite with minor suspended crystals [4]
    ...
    msl.set_temperature_mode('adiabatic')
    ...
    mantle_radii = np.linspace(1841.645e3, 2830.303e3, 100) #S changed radius from Kh22
...
#    )
    ...
    lithosphere = burnman.Layer("lithosphere", radii = np.linspace(2830.303e3, 3346.303e3, 100))
    ...
    lithosphere.set_temperature_mode("user-defined", np.linspace(1650, 331, 100)) #S
    ...
    planet_mars = burnman.Planet(
        "Planet Mars", [core, msl, mantle, lithosphere, crust], verbose=True     
    )
    print(planet_mars)
