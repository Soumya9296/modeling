...
if __name__ == "__main__":
    ...
    #core_composition = Composition({'Fe': 81.00, 'Ni': 3.45, 'S': 12.50, 'Si': 0.00, 'O': 3.00, 'C': 0.025, 'H': 0.025}, 'weight') # fig 4 layer 1

    core_composition = Composition({'Fe': 79.50, 'Ni': 7.40, 'Co': 0.33, 'S': 6.60, 'Si': 0.00, 'P': 0.33, 'O': 5.20, 'C': 0.00, 'H': 0.9}, 'weight') # YM20
    
    #core_composition = Composition({'Fe': 79.00, 'Ni': 7.50, 'S': 13.25, 'Si': 0.00, 'O': 4.20, 'C': 0.025, 'H': 0.025}, 'weight')
    ...
    core.set_temperature_mode("adiabatic", temperature_top = 2350.0) #s 2100 frm S19 in Kh22   2550
    ...
    tbl_perturbation = burnman.BoundaryLayerPerturbation(
        ...
        boundary_layer_ratio=100/300, #s changed as sluggish mantle with top-heavy thermal structure  # 0.0 means No boundary layer i.e. uniform perturbation
    )
    ...
    lithosphere.set_temperature_mode("user-defined", np.linspace(1600, 331, 100)) #S
    ...
    cmb_index = max(i for i, name in enumerate(planet_mars.layer_names) if name == "mantle")
